	...
	useEffect(() => {
		fetch(`http://localhost:3333/users/${user}`)
			...
			.then((json) =>
				dispatch({
					type: 'name',
					payload: json.first_name + ' ' + json.last_name,
				})
			)
	...
	if (user == null) {
		...
		return (
			<div>
				...
				<Button className="btn-lg btn-dark btn--block back" onClick={Posts}>
					Post
				</Button>
				<Button
				...
				<Button className="btn-lg btn-dark btn--block back1" onClick={Profile}>
					Profile
				</Button>
